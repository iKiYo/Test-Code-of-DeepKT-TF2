...
class DKTAccumBIModel(tf.keras.Model):
  ...
  def __init__(self, num_students, num_skills, max_sequence_length,
    ...
    mask = layers.Masking(mask_value=0.0, input_shape=(None, embed_dim*2))
    ...
    # *** concatenations
    embed_c = c_emb(c_concat([log_count, log_delta])) # 2K+1 to N
    x_c = c_concat([embed_x, embed_c]) # N + N
    elm_square = x_c * x_c
    form_tensor = tf.constant([1,1,1, embed_dim*2], tf.int32) 
    x_c= tf.tile(tf.expand_dims(x_c, -1), form_tensor)
    x_c_T = tf.transpose(x_c,perm=[0,1,3,2])
    squared_sum = tf.reduce_sum(x_c * x_c_T, axis=-1)
    x_c_t = 0.5 * (squared_sum - elm_square)
    ...
    super().__init__(inputs=[x, delta, q], outputs=outputs, name="DKTAccumBIModel")
