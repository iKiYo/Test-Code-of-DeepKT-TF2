...
class DKTAccumDotConcatModel(tf.keras.Model):
  ...
  def __init__(self, num_students, num_skills, max_sequence_length,
    ...
    x_c_t = c_concat([c_dot([embed_x, embed_c]), log_count , log_delta]) # N + 2K + 1
    ...
    super().__init__(inputs=[x, delta, q], outputs=outputs, name="DKTAccumDotConcatModel")
