...
def train_model(outfile_path, model, train_dataset, val_dataset, hparams, 
  ...
  early_stop_callback = tf.keras.callbacks.EarlyStopping(monitor=monitor_name, patience=5, 
                                                                                                                    mode='max')
  reduce_lr_plateau_callback = tf.keras.callbacks.ReduceLROnPlateau(
    monitor=monitor_name, factor=0.1, patience=3, verbose=0, mode='auto',
    min_delta=0.0000, cooldown=0, min_lr=0
)
