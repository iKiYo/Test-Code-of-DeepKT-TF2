...
class Decoder(tf.keras.layers.Layer):
  def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size,
    ...
    # skill ID begins from zeros and added 1 to all 
    ...
    # concat version
    self.embedding = tf.keras.layers.Embedding(target_vocab_size, d_model//2, mask_zero=True) # skill ID begins from zeros and added 1 to all 


    # addition of delta_t
    # self.delta_embedding = tf.keras.layers.Embedding(delta_duration, d_model, mask_zero=True)
    # concat version
    self.concat = tf.keras.layers.Concatenate()
    self.dot = tf.keras.layers.Multiply()
    self.delta_embedding = tf.keras.layers.Dense(d_model//2)
    # self.delta_embedding = tf.keras.layers.Dense(d_model)
    # self.delta_embedding = tf.keras.layers.Dense(d_model, kernel_constraint=tf.keras.constraints.non_neg,
                                                #  activation='exponential') # with exponential
  ...
  def call(self, x, delta_t, enc_output, training, 
           look_ahead_mask, padding_mask):
    ...
    # delta_t = self.delta_embedding(delta_t) # row + expo activation
    # tf.debugging.assert_negative(delta_t)
...
class SAINT_tempo(tf.keras.Model):

  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, 
               target_vocab_size, pe_input, pe_target, rate=0.1):
    super(SAINT_tempo, self).__init__(name="SAINT_tempo")
  ...
  def call(self, data, training=True): 
    # inp, tar = data['x'], data['q']
    # tuple version
    # inp, tar = data
    # tuple version with lag_time data
    inp, tar, delta_t = data
    ...
    dec_output, attention_weights = self.decoder(
        tar, delta_t, enc_output, training, look_ahead_mask, dec_padding_mask)
