...
class DKTAccumDotModel(tf.keras.Model):
  ...
  def __init__(self, num_students, num_skills, max_sequence_length,
    ...
    # *** concatenations
    embed_c = c_emb(c_concat([log_count, log_delta])) # 2K +1
    x_c_t = c_dot([embed_x, embed_c]) # N
    ...
    super().__init__(inputs=[x, delta, q], outputs=outputs, name="DKTAccumDotModel")
